...
module.exports = CoreObject.extend({
  init: function(options) {
    ...
    const deployAlways = this.plugin.readConfig('deployAlways') || [];
    s3Options.deployAlways = deployAlways;
  ...
  _determineFilePaths: function(options) {
    ...
    const deployAlways = options.deployAlways;
    ...
    if (manifestPath) {
      ...
      return new RSVP.Promise(function(resolve, reject){
        ...
        var paths = _.difference(filePaths, manifestEntries);
        return paths.concat(deployAlways);
